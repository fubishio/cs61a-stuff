...
def shuffle(cards):
    ...
    halfwaypoint = (len(cards))//2
    firsthalf = cards[0:halfwaypoint]
    secondhalf = cards[halfwaypoint:]
    finallist = []
    n = 0
    while n < (halfwaypoint):
        finallist.append(firsthalf[n])
        finallist.append(secondhalf[n])
        n += 1
    return finallist
...
def has_prefix(s, prefix):
    ...
    if s == empty:
        return False
    if first(prefix) != first(s):
        return False
    if rest(prefix) == empty:
        return True
    return has_prefix(rest(s), rest(prefix))
    # if first(s) != first(prefix):
    #     return False
    # elif first(prefix) == empty:
    #     return True
    # else:
    #     return has_prefix(rest(s), first(rest(prefix)))
    
    # def evalmethod(prefix):
    #     lis = []
    #     if s == empty:

    #     while prefix != empty:
    #         if is_link(rest(prefix)):
    #             lis.append(first(rest(prefix)))
    #     return lis
    # complen = len(evalmethod(prefix))
    # c = s[complen:]
    # if c == evalmethod(prefix):
    #     return True
    # else:
    #     return False
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    def passing(balance, banana):
        if (banana == new_password) or (banana == old_password):
            return withdraw(balance, old_password)
        else:
            return withdraw(balance, banana)
