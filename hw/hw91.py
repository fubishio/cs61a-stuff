...
def tokenize(line):
    ...
    for t in line.split():
        "*** YOUR CODE HERE ***"
        if t not in all_brackets and coerce_to_number(t) is None:
            raise ValueError('invalid token ' + str(t))
        elif t in all_brackets:
            tokens = tokens + [t]
        else:
            tokens = tokens + [coerce_to_number(t)]
    return tokens
...
def brack_read(tokens):
    ...
    if n != None:
        ...
        "*** YOUR CODE HERE ***"
        t = (token,left_to_right[token])
        operator = brackets[t]
        return Pair(operator,read_tail(tokens,left_to_right[token]))
    else:
...
class Mobile:
    ...
    def is_balanced(self):
        ...
        "*** YOUR CODE HERE ***"
        v = self.right.torque == self.left.torque
        for i in [self.right.contents,self.left.contents]:
            if isinstance(i,Mobile):
                v = v and i.is_balanced()
        return v
...
def make_s():
    ...
    def rest():
        "*** YOUR CODE HERE ***"
        i = scale_stream(s,2)
        p = merge(scale_stream(s,3),scale_stream(s,5))
        return merge(i,p)
    s = Stream(1, rest)
    return s
def filter(fn,s):
...
def unique(s):
    ...
    def helper(x):
        return lambda k : k != x
    def rest():
        return unique(filter(helper(s.first),s.rest))
    return Stream(s.first,rest)
...
def rle(s, max_run_length=10):
    ...
    first = s.first
    s = s.rest
    while s != Stream.empty and s.first == first and timer < max_run_length:
        timer+=1
        s = s.rest
    return Stream((timer,first),lambda :rle(s,max_run_length))
